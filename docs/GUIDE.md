...
## How-to: reports — read, download, and share what a run produced

Every run becomes a self-contained Markdown report, generated by Windhover from the
run's own node outputs — no work required in your graph. If a node wrote an LLM
narrative, a list of findings, or a summary object, the report surfaces it: prose as
prose (leading with the run's main narrative), record lists as tables, timings per step.

- **Reports view** (left nav) lists runs with one-click **👁 Report** (rendered preview)
  and **⬇ .md** (download). Filter to **Needs attention** (failed · awaiting approval ·
  flagged), **Produced files**, or **All**. It's the place to review and share results
  without opening the trace — the `.md` downloads and shares like any document.
- **In a run's drawer**, the same **⬇ Report (.md)** and **👁 Preview report** buttons
  sit next to Replay.
- **Files a run wrote** (a PDF, an .xlsx, a chart) are detected automatically from any
  absolute path in its node outputs and appear as downloadable artifacts — return the
  path in node state, nothing else needed.
- **Capture size:** outputs are captured up to `WINDHOVER_MAX_OUTPUT` bytes (default
  24000) so a full report isn't clipped. Raise it if your nodes emit larger text.
...
- **Inline preview**: HTML (sandboxed iframe - scripts never execute), PDF, images/SVG,
  Python and other code (highlighted), CSV/TSV (as a table), JSON/text, **rendered
  Markdown**, **audio and video** (native players), **Word** (.docx via mammoth),
  **Excel** (.xlsx/.xls via SheetJS), and **PowerPoint** (.pptx as a per-slide text
  outline via jszip). The heavier renderers are vendored and lazy-loaded only when you
  preview one, and their output shows in the same sandboxed frame. **Download-only**:
  legacy .doc/.ppt, archives, and model/data binaries (.parquet, .pt, .npz, …). Only
  recognized extensions are surfaced at all, and every artifact is downloadable.
...
- The top-bar selector also offers **All graphs** (multi-graph only): Stats shows
  deployment-wide totals with per-node latency rows grouped per graph so same-named
  nodes never merge, Sessions lists every graph a session touched, and Runs shows
  everything. The graph canvas and Memory fall back to your last concrete graph (the
  canvas notes which) - memory namespaces belong to one graph's store, and a topology
  canvas is per-graph by definition.
